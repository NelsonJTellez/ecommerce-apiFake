...
export function renderizarPaginaSesion(contenedor: HTMLElement) {
  ...
  formLogin.onsubmit = (e) => {
    ...
    if (!validarContraseña(contrasena)) {
      errorLogin.textContent =
        "La contraseña debe tener mínimo 9 caracteres, incluyendo mayúsculas, minúsculas y un símbolo.";
      return;
    }
  ...
  formRegister.onsubmit = (e) => {
    ...
    if (!usuario) {
      errorRegister.textContent = "El usuario es obligatorio.";
      return;
    }
    if (!validarContraseña(contrasena)) {
      errorRegister.textContent =
    ...
    // Aquí puedes añadir lógica para guardar usuario (localStorage o backend)
    Sesion.obtenerInstancia().iniciarSesion(usuario);
    location.hash = "#/";
  };
}
